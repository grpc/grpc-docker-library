...
```dockerfile
FROM grpc/python:1.14.1-onbuild
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp grpc/python:1.14.1 python your-grpc-python-client-or-server.py
```
